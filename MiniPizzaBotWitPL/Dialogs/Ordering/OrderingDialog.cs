...
namespace MiniPizzaBot
{
    public class OrderingDialog: ComponentDialog
    {
        ...
        private async Task<bool> ValidatePizzaName(PromptValidatorContext<string> promptContext, CancellationToken cancellationToken)
        {
            ...
            {
                await promptContext.Context.SendActivityAsync($"Nazwa pizzy musi mieć przynajmniej `{PizzaNameLengthMinValue}` znaków.");
                return false;
            }
        }
        ...
        {            
            ...
            {
                await promptContext.Context.SendActivityAsync($"Ilość kawałków pizzy musi zawierać się pomiędzy `{MinPizzaPieces}` i `{MaxPizzaPieces}`.");
                return false;
            }
        }
        ...
        private async Task<DialogTurnResult> PizzaOrderSummary(WaterfallStepContext stepContext)
        {
            ...
            await context.SendActivityAsync($"Twoje zamówienie: {orderingState.PizzaName} {orderingState.PizzaPieces}");
